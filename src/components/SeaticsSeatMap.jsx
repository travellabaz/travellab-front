...
//
// Plain width:100%, no fixed-size + CSS transform scaling — that was
// tried to dodge Seatics' mobile/desktop CSS breakpoint, but TicketNetwork's
// own integration guide says explicitly not to fight their sizing:
// "Remove any previous references to the MapWidget stylesheet. The API
// will automatically add the styles needed for the map and its
// components" (Guide - Updating to the MapWidget3 API v2.1, TicketNetwork
// 2017) — the widget is meant to size itself. The transform-scale
// version also coincided with the map's own SVG rendering itself
// thousands of pixels below the visible area on some events (confirmed
// live: computed top offset in the hundreds of thousands of pixels) —
// plausibly the widget reading its own frameElement's scaled
// getBoundingClientRect() for a zoom-to-fit calculation and dividing by
